...
You can also enable or disable which Code Actions are run on save per language using a [language specific setting](https://code.visualstudio.com/docs/configure/settings#_language-specific-editor-settings). The following settings enable organize imports on save for TypeScript files only:
...
* Run a script as a [task](https://code.visualstudio.com/docs/debugtest/tasks) with the output shown in the Integrated Terminal.
